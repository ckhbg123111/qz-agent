...
| 文档版本 | v1.0 |
| 文档状态 | 可联调 |
| 适用对象 | 医院信息科、HIS/LIS/RIS 对接公司、实施工程师 |
| 更新日期 | 2026-03-17 |
| 对接方式 | HTTPs API（JSON） |
...
| 项目 | 测试环境 | 生产环境 |
| --- | --- | --- |
| Base URL | 待医院提供 | 待医院提供 |
| 网络策略 | 白名单互通 | 白名单互通 |
| TLS 版本 | TLS 1.2+ | TLS 1.2+ |
| 字符集 | UTF-8 | UTF-8 |
...
| 角色 | 姓名 | 联系方式 | 职责 |
| --- | --- | --- | --- |
| 医院业务负责人 | 待补充 | 待补充 | 业务口径确认 |
| 医院技术负责人 | 待补充 | 待补充 | 接口联调与上线 |
| 平台实施负责人 | 待补充 | 待补充 | 联调排期与问题跟踪 |
...
| 接口路径 | 方法 | 业务含义 | 备注 |
| --- | --- | --- | --- |
| `/lab-appointment` | POST | 检验预约推送 | 兼容现网幽门螺杆菌场景 |
| `/report` | POST | 检验报告推送 | 兼容现网幽门螺杆菌场景 |
| `/prescription` | POST | 处方开具推送（含随访任务） | 兼容现网幽门螺杆菌场景 |
...
建议使用双层鉴权（推荐）：
- 第一层：网关签名鉴权（`X-Access-Key` + `X-Signature`）
- 第二层：JWT 鉴权（`Authorization: Bearer <token>`，按院方安全策略启用）

注意：
- 若院方网关已统一完成 JWT 校验，可在应用层仅保留签名鉴权。
- 无论是否启用 JWT，签名鉴权均建议保留。
...
1. 双方确认字段映射表、字典值、状态流转与鉴权方式。  
2. 在测试环境完成连通性验证（签名、时间窗、防重放）。  
3. 按接口顺序进行全链路联调：挂号 -> 检查单 -> 预约 -> 完成 -> 报告 -> 病例 -> 处方。  
4. 进行异常链路联调：取消、改约、更正报告、停嘱/撤方。  
5. 进行压测与回放：验证峰值吞吐、消息重试、幂等去重。  
6. 输出《联调验收记录》并冻结字段版本后再上线生产。
...
为便于医院对接公司执行，建议按以下清单交付：

| 输出物 | 内容 | 责任方 |
| --- | --- | --- |
| 接口字段映射表 | 本文第 3、5、8 节字段落地到 HIS/LIS/RIS 字段 | 双方 |
| 鉴权参数表 | AccessKey、Secret、时间窗、防重放策略 | 平台方/医院方 |
| 联调测试用例 | 正常流程 + 异常流程 + 重试幂等 | 双方 |
| 联调验收记录 | 每个接口的请求样例、响应结果、问题闭环 | 双方 |
| 上线切换方案 | 灰度范围、回滚策略、联系人值班表 | 双方 |
