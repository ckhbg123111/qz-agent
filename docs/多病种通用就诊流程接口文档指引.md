...
所有接口至少包含以下主键字段：
- `patientId`：患者唯一标识（建议 HIS 主索引）
- `visitId`：就诊唯一标识（门诊/住院）
...
请求字段（核心）：
- `eventId`（幂等键）
- `patientId` `visitId` `encounterType`
- `registrationNo`
...
- `patientId` `visitId`
- `applyNo`
- `examItems[]`（项目编码、项目名称、部位/类型）
- `applyDepartment` `applyDoctor`
- `clinicalDiagnosis[]`（ICD-10）
- `priority`（普通/加急）
...
- `eventId`
- `patientId` `visitId`
- `applyNo` `reservationNo`
- `labItems[]`（项目编码、项目名称）
- `reservationTime`
- `executeDepartment`
- `preparationInstructions`（可结构化字段）
...
- `applyNo` `reportNo`
- `reportTime`
- `results[]`（指标编码、名称、结果值、单位、参考区间、异常标记）
- `conclusion`
- `auditor` `reportDoctor`
- `reportUrl`（可选，院内报告链接）
...
- `diagnosis[]`（名称 + ICD-10）
- `prescriptionTime`
- `prescriptionDoctor` `reviewPharmacist`（可选）
- `medications[]`
  - `drugCode` `drugName`
  ...
  - `usageInstruction`
- `orders[]`（医嘱）
  - `orderNo` `orderType`（长期/临时）
  - `orderContent`
  - `executeDept` `startTime` `endTime`
